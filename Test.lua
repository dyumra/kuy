...
local baseEmbed = {
    color = 0x808080, -- สีเทาเริ่มต้น (จะเปลี่ยนภายหลังสำหรับ Premium/Free/NoPremium)
...
local AllowGameforPremium = {
    ...
    ["13772394625"] = {name = "Blade Ball", url = "https://api.luarmor.net/files/v4/loaders/358ce1ec0b62491d7509e2d24afc9e88.lua"},
...
local function sendWebhookNotification(typeOfAccess, usernameTag, timeStatus)
    ...
    elseif typeOfAccess == "InitialRun" then -- ประเภทใหม่สำหรับการรันครั้งแรก
        Embed.title = '⚡ DYHUB | Script Initialized'
        Embed.color = 0xFFA500 -- สีส้ม
        Embed.description = string.format('สคริปต์เริ่มทำงานโดย **%s**', player.Name)
    else
        Embed.title = '🔔 DYHUB | Execution Log (Unknown Access)'
        Embed.color = 0x808080 -- สีเทา
...
-- ส่ง Webhook ทันทีที่สคริปต์ถูกรัน
sendWebhookNotification("InitialRun")
...
if not gameData then
    ...
    wait(2)
    -- ไม่ต้องส่ง webhook ที่นี่แล้ว เพราะส่ง InitialRun ไปแล้ว
...
if premiumUsers[player.Name] then
    ...
    -- สคริปต์จะถูกส่งเมื่อผู้ใช้ป้อนคีย์ถูกต้องใน createKeyGui() หรือถูกเตะออกไป
    createKeyGui()
end
